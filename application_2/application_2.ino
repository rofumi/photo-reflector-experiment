...
void loop() {
  ...
  //LEDの点灯有無センサ値の差分（LED光のみ） & ローパスフィルター
  ...
  //=== サーボモータの制御
  if(control_value > 600){
    tar_angle = SERVO_ANGLE_MAX * 2.0* (control_value - 600) / LIGHT_VALUE_MAX;
    if(tar_angle > SERVO_ANGLE_MAX)   tar_angle = SERVO_ANGLE_MAX;
  }else{
    tar_angle = 0.0;
  }
